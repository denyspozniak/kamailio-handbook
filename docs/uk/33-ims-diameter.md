...
- **Peer flapping.** Якщо з'єднання до HSS/DRA падає, `cdp` зносить його і ретраїть; у це вікно кожен REGISTER провалюється (немає MAA). Peer, що мовчить на DWR-watchdog'ах, втрачатиме реєстрації — алертіть на CER/DWR-стан.
- **Розбіжність AVP-словників.** `cdp` розуміє лише AVP із завантажених словників. Vendor-specific-AVP, якого словник не має, дропається або відхиляє повідомлення, виринаючи як SIP-side-помилка далеко від причини.
- **Блокування воркера.** *Синхронний* Diameter-виклик (неправильна async-обв'язка) паркує SIP-воркера на весь таймаут HSS; на registration storms це каскадить. Машинерія suspend/continue існує, щоб цьому запобігти.
