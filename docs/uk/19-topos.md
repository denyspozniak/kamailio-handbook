...
`topos` тримає mapping у shm, проіндексований за token'ом, який вбудовує у переписані заголовки. Таблиця тримає:
...
Це також показує цінність `dialog`. Природне місце для mapping'у — проіндексованого за dialog-ідентифікаторами — а `dialog` вже тримає record per call. `topos` може повісити свої дані на існуючий dialog-record (або тримати паралельну структуру, залежно від конфігу).
