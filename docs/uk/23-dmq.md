...
> [!NOTE]
> Це приземлилося через master-лінію — операційний polish від людей, що ганяють великі `dmq`-кластери в бою (більшість пачки написав Влад Литвинов з net2phone). Перевіряйте версію модуля, перш ніж покладатися на будь-що з цього — `kamcmd core.version` і README модуля.
