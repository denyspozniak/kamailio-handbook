...
**`onreply_route`** — стріляє на кожну вхідну **відповідь**, до того, як її перешлють назад UAC. Голий `onreply_route { … }` бігає беззастережно; іменовані варіанти `onreply_route[N]` бігають лише на відповіді тих транзакцій, що opt-in'ули через `t_on_reply("N")`. Іменовані — це спосіб перехопити відповідь конкретного виклику, який ви проксіювали (для SDP rewriting, accounting на 200 OK тощо).
